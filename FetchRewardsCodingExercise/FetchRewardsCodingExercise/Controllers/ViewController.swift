...
extension ViewController: UISearchBarDelegate {
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        // Else go through all the events, then filtering
        else {
            for event in listOfEvents {
                // First Filter based off included words
                if event.title.lowercased().contains(searchText.lowercased()) {
                    filteredData.append(event)
                }
                // Then Filter based off event taxonomy or classification if first filter fails.
                // this nested for loop will only run if it needs to in the worst case scenario.
                
                // non production, this will work fine, but if data is large during production time, the time complexity will matter. but since we are only dealing will 10 event objects at a time, this should suffice
                else {
                    for taxonomy in 0...event.taxonomies.count-1 {
                        if event.taxonomies[taxonomy].name.lowercased().contains(searchText.lowercased()) {
                            filteredData.append(event)
                        }
                    }
                }
